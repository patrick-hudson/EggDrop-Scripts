...
proc webby {nick uhost handle chan site} {
  ...
  regsub -all {(?:\n|\t|\v|\r|\x01)} $html " " html
     ...
     close $junk
     set title ""
        if {[string match *twitter.com/*/status/* $url]} {
          while {[string match 30* $title]} { 
            regexp -nocase -- {<a href="(.*?)">} $html - title
        ...
        if {[regexp -nocase {<div class="body-content">(.*?)} $html]} {
         ...
         if {[regexp -nocase {<p class="js-tweet-text tweet-text" .*?>(.*?)</p>} $html ltweet]} {
           #set ago ""
            ...
            #regsub -all -nocase -- {<span class=".*?">(.*?)</span>} $ltweet "\\1 " ltweet
            #regsub -all -nocase -- {<span class="Icon Icon--verified Icon--small"><span class="u-hiddenVisually">Verified account</span></span>} $ltweet "" ltweet
            #regsub -all -nocase -- {<a >(.*?)</a>} $ltweet "" ltweet
            #regsub -all -nocase -- {<ul .*?>(.*?)</ul>} $ltweet "" ltweet
            #regsub -all -nocase -- {<li .*?>(.*?)</li>} $ltweet "" ltweet
            #regsub -all -nocase -- {<b>(.*?)</b>} $ltweet "" ltweet
            regexp -nocase -- {<div class="client-and-actions">(.*?)</div>} $html ago
            regsub -all -nocase -- {<div .*?>(.*?)</div>} $ago "\\1" ago
            regsub -all -nocase -- {<span>(.*?)</span>} $ago "\\1" ago
            regsub -all -nocase -- {<span .*?>(.*?)</span>} $ago "\\1" ago
            regsub {^[\ ]*} $ago "" ago
            regsub {[\ ]*$} $ago "" ago
            
            regexp -nocase {<span class="username js-action-profile-name".*?>(.*?)</span>} $html real
            regsub -all -nocase -- {<span class="username js-action-profile-name".*?>(.*?)</span>} $real "\\1" real
            regsub -all -nocase -- {<s>(.*?)</s>} $real "\\1" real
            regsub -all -nocase -- {<b>(.*?)</b>} $real "\\1" real
            set title "$real: $ltweet at $ago"
         }
     } elseif {![regexp -nocase {<title.*?>(.*?)</title>} $html - title]} {
        if {[info exists w10]} {
